...
## Figure 5 – NDIS Anomaly Distribution

```{r}
#| label: fig5-anomaly-distribution
#| echo: true
#| fig-width: 14
#| fig-height: 16
...
ggsave(
  filename = file.path(figures_dir, "fig5_anomaly_distribution.pdf"),
  plot = final_plot,
  width = 14,
  height = 16,
  dpi = 300,
...
ggsave(
  filename = file.path(figures_dir, "fig5_anomaly_distribution.png"),
  plot = final_plot,
  width = 14,
  height = 16,
  dpi = 300,
...
## Figure 6 – Time Gap Analysis

This figure is generated by the `ndis_analysis.qmd` pipeline and shows the temporal gaps in the NDIS snapshot data.

```{r}
#| label: fig6-time-gap-analysis
#| echo: false
#| fig-alt: "Four-panel figure showing time gap analysis: (a) monthly snapshot counts, (b) yearly variance, (c) gap distribution with outliers, (d) gap distribution without outliers."

knitr::include_graphics(file.path(figures_dir, "fig6_time_gap_analysis.png"))
```
...
# Load flagged time series data (long format with flags)
ndis_clean <- read_csv(file.path(here("data", "ndis", "final", "ndis_time_series.csv")), show_col_types = FALSE)

# Filter out flagged data and pivot to wide format for aggregation
ndis_clean_wide <- ndis_clean %>%
  filter(
  ...
  select(capture_datetime, jurisdiction, metric_type, value) %>%
  pivot_wider(names_from = metric_type, values_from = value, values_fn = max)

growth_data_yearly <- ndis_clean_wide %>%
  ...
  # Handle Inf values from max on empty groups
  mutate(across(c(offender_profiles, arrestee, forensic_profiles, investigations_aided), 
