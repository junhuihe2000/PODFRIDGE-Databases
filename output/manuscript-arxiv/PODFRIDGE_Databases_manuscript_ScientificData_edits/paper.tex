...
\author[1,4,*]{%
  \href{https://orcid.org/0000-0002-7546-6724}{\usebox{\orcid}\hspace{1mm}Tina Lasisi}%
}
...
pdfauthor={Pryor, Y., Donadio, J. P., Muller, S.C., Wilson, J., Lasisi, T.},
...
Using these rules, we identified 898 flags across all metrics and jurisdictions (Figure~\ref{fig:anomalies}). The majority were stale exact reappearances ($743$), followed by spike-dip events ($77$), decimal spike-dip errors ($68$), and NDIS labs spikes ($10$). By metric, offender profiles had the most flags ($420$), followed by forensic profiles ($379$), investigations aided ($49$), arrestee profiles ($40$), and NDIS laboratories ($10$). Figure~\ref{fig:heatmaps} shows the temporal coverage of NDIS snapshots and the distribution of time gaps between captures.
...
\caption{\textbf{Anomaly flag distribution.} A four-panel stacked bar chart showing the distribution of data quality flags across jurisdictions, broken down by flag type: decimal spike-dip, spike-dip, NDIS labs spike, and stale exact reappearance. Each panel shows the count of flags per jurisdiction for a given flag category, colored by metric type.}
\label{fig:anomalies}
\end{figure}
...
\caption{\textbf{NDIS time gap analysis.} Heatmaps and distributions showing the temporal coverage of NDIS snapshots across jurisdictions and the gaps between successive captures, illustrating the density and regularity of archived FBI data availability over time.}
\label{fig:heatmaps}
\end{figure}
