...
\author[1]{%
  \href{https://orcid.org/0009-0005-5151-5068}{Virum Ranka}%
}
\author[2]{%
...
The Combined DNA Index System (CODIS) is a software platform originated by the FBI to store, compare, and search DNA profiles using a standardised set of short tandem repeat (STR) loci$^{1}$. CODIS is implemented across three levels: local DNA index systems (LDIS) maintained by individual laboratories, state DNA index systems (SDIS) operated by state agencies, and the National DNA Index System (NDIS) administered by the FBI. DNA profile records flow upward from LDIS to SDIS to NDIS if they meet defined eligibility criteria and are organised into multiple indices, two of which are the Offender Index (which includes profiles from convicted individuals and, where authorized, arrestees) and the Forensic Index, which contains crime scene profiles of unknown offenders$^{2}$.
...
\textbf{Decimal spike-dip} (\texttt{flag\_decimal\_spike\_dip}). Applied to all metrics. This rule detects suspected decimal place entry errors by flagging values where $N_{j,t}^{(x)} > 8 \times \text{baseline}$ or $N_{j,t}^{(x)} < 0.125 \times \text{baseline}$, provided recovery toward the baseline occurs within the recovery window.

\textbf{Spike-dip} (\texttt{flag\_spike\_dip}). Applied to offender profiles, forensic profiles, arrestee profiles, and investigations aided. This rule flags sudden doublings or halvings: $N_{j,t}^{(x)} > 2 \times \text{baseline}$ or $N_{j,t}^{(x)} < 0.5 \times \text{baseline}$, again requiring recovery within the window.

\textbf{NDIS labs spike} (\texttt{flag\_ndis\_labs\_spike}). Applied to NDIS laboratory counts only. Because lab counts are small integers (typically 1--20), a lower threshold is used: $N_{j,t}^{(x)} > 3 \times \text{baseline}$ or $N_{j,t}^{(x)} < 0.33 \times \text{baseline}$.

\textbf{Stale exact reappearance} (\texttt{flag\_stale\_exact\_reappearance}). Applied to offender profiles, forensic profiles, and arrestee profiles. This rule flags instances where an exact earlier value reappears after higher values have been recorded, indicating cached or stale data served by the FBI website. Consecutive identical values are not flagged, as these represent legitimate unchanged counts between reporting periods.
...
\item FBI. CODIS Archive. \url{https://le.fbi.gov/science-and-lab/biometrics-and-fingerprints/codis-2} (2022).

\item National Institute of Justice. Forensic DNA Education for Law Enforcement Decisionmakers. \url{https://nij.ojp.gov/nij-hosted-online-training-courses/forensic-dna-education-law-enforcement-decisionmakers/partial-matches/codis-hierarchy-cont} (Accessed: 2025).

\item FBI. CODIS-NDIS Statistics. \url{https://le.fbi.gov/science-and-lab/biometrics-and-fingerprints/codis/codis-ndis-statistics} (2022).

\item Ge, J., Eisenberg, A. \& Budowle, B. Developing criteria and data to determine best options for expanding the core CODIS loci. \textit{Investig. Genet.} \textbf{3}, 1 (2012). \url{https://doi.org/10.1186/2041-2223-3-1}

\item Ge, J. DNA based familial searching and related statistical issues. \textit{J. Forensic Res.} \textbf{3}, e112 (2012). \url{https://doi.org/10.4172/2157-7145.1000e112}

\item Ge, J. et al. Future directions of forensic DNA databases. \textit{Croat. Med. J.} \textbf{55}, 163--166 (2014). \url{https://doi.org/10.3325/cmj.2014.55.163}

\item Wickenheiser, R. A. Expanding DNA database effectiveness. \textit{Forensic Sci. Int. Synerg.} \textbf{4}, 100226 (2022). \url{https://doi.org/10.1016/j.fsisyn.2022.100226}

\item Link, V. et al. Microsatellites used in forensics are in regions enriched for trait-associated variants. \textit{iScience} \textbf{26}, 107992 (2023). \url{https://doi.org/10.1016/j.isci.2023.107992}

\item Greenwald, E. \& Phiri, L. On Accountability: Genetic Tools for Justice and Injustice in Criminal Proceedings. \textit{J. Sci. Policy Gov.} \textbf{25}, 0109 (2024). \url{https://doi.org/10.38126/JSPG250109}

\item Murphy, E. \& Tong, J. The racial composition of forensic DNA databases. \textit{Calif. Law Rev.} \textbf{108}, 1847--1930 (2020).

\item Hares, D. R. Selection and implementation of expanded CODIS core loci in the United States. \textit{Forensic Sci. Int. Genet.} \textbf{17}, 33--34 (2015). \url{https://doi.org/10.1016/j.fsigen.2015.03.006}

\item Pryor, Y., Ranka, V., Donadio, J. P., Muller, S., Wilson, J. \& Lasisi, T. United States forensic DNA databases: NDIS, SDIS, and FOIA datasets. Zenodo \url{https://doi.org/10.5281/zenodo.18820515} (2026).
